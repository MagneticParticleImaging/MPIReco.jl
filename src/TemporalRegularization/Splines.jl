...
mutable struct TempRegSplinesOperator{V<:AbstractMatrix}
  S::V
  Φ::V
  N::Int
  M::Int
...
function reconstructionTempRegSplines(S, bSF::Union{T,Vector{T}}, bMeas::MPIFile, freq::Array, grid;
  ...
  Op = TempRegSplinesOperator(Float32(1/(sqrt(λ)))*S, Float32(1/(sqrt(β)))*bgDict, L; 
                                      interpMeth, idxFG, idxBG )

  global Top = Op

  c = reconstructionTempRegSplines(Op, u; solver, λ, β, shape=shape(grid), kargs...)
...
fashion
"""
function reconstructionTempRegSplines(Op::TempRegSplinesOperator, u::Array; λ, β,
                        solver = "kaczmarz", enforceReal=false, enforcePositive=false,
                        backgroundCoefficients = nothing, kargs...)
...
getSizes(Op::TempRegSplinesOperator) = 
      Op.M÷Op.L, size(Op.S,2), size(Op.Φ,2), length(Op.idxFG), length(Op.idxBG) 
 
Base.eltype(TempOp::TempRegSplinesOperator) = eltype(TempOp.S)

function size(TempOp::TempRegSplinesOperator, i::Int)
  if i==2
    return TempOp.N
  elseif i==1
    return TempOp.M
...
size(TempOp::TempRegSplinesOperator) = (TempOp.M,TempOp.N)
length(TempOp::TempRegSplinesOperator) = TempOp.M*TempOp.N

function TempRegSplinesOperator(S, Φ, L; interpMeth=:linear, idxFG=1:L, idxBG=1:L )
  ...
  N = size(S,2)*length(idxFG)+size(Φ,2)*length(idxBG)
  M = size(S,1)*L

  return TempRegSplinesOperator(S, Φ, N, M, L, idxFG, idxBG, 
                           idxCoeffsFG, idxCoeffsBG, coeffsFG, coeffsBG)
end
...
function calculateTraceOfNormalMatrix(Op::TempRegSplinesOperator, weights)
  return calculateTraceOfNormalMatrix(Op.S, weights)
end

setlambda(::TempRegSplinesOperator, ::Any) = nothing

getSubIdxRows(k,M) = mod1(k,M), (k-1)÷M+1

function RegularizedLeastSquares.dot_with_matrix_row(Op::TempRegSplinesOperator, 
...
function RegularizedLeastSquares.kaczmarz_update!(Op::TempRegSplinesOperator, 
  ...
  for κ=1:length(Op.idxCoeffsFG[l])
    ...
    @simd for n = 1:NSub
      @inbounds xFG[n,Op.idxCoeffsFG[l][κ]] += α * conj(Op.S[i,n])
    end
  end
  ...
  for κ=1:length(Op.idxCoeffsBG[l])
    ...
    @simd for q = 1:Q
      @inbounds xBG[q,Op.idxCoeffsBG[l][κ]] += α * conj(Op.Φ[i,q])
    end
  end
  return
end

function RegularizedLeastSquares.initkaczmarz(Op::TempRegSplinesOperator,λ,weights::Vector)
